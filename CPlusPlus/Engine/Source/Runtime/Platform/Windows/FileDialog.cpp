...
namespace Engine::Windows
{
	...
	static Nullable<U8String> OpenDialog(const DynamicArray<U8String>& filters, const U8String& initialDirectory, const U8String& title, Void* owner, Callback Fn)
	{
		constexpr SizeType size = 512;
		Char16 buffer[512];
		MemoryClear( buffer, sizeof(Char16) * size );
	...
	Nullable<U8String> FileDialog::Open(const DynamicArray<U8String>& filters, const U8String& initialDirectory, const U8String& title, Void* owner)
	{
		return OpenDialog(filters, initialDirectory, title, owner, GetOpenFileNameW);
	}

	Nullable<U8String> FileDialog::Save(const DynamicArray<U8String>& filters, const U8String& initialDirectory, const U8String& title, Void* owner)
	{
