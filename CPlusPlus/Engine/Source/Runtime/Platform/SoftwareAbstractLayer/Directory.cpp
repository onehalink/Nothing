...
namespace Engine
{
	...
	{
		ASSERT( Exist(from), Error::DirectoryNotFound );
	...
	{
		ASSERT( !Exist(path), Error::DirectoryExisted );
		...
		{
			Int32 startIndex	= FindLast<Char8>(entryWalker, PreferedPathSeparator) + 1;
	...
	{
		ASSERT( Exist(path), Error::DirectoryNotFound );
	...
	Array<U8String> Directory::EnumerateEntries(const U8String& path, SearchOption searchOption)
	{
		ASSERT( Exist(path), Error::DirectoryNotFound );
