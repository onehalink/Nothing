...
namespace Engine::UI
{
    ...
    Void ScrollBar::Update(const InputSystem& inputSystem)
    {
        ...
        Bool isHover = IsInside(grab.rectangle, inputSystem.mouse.position);
        ...
        if (isHover && isPress && !grab.holded)
        {
            ...
            grab.beginDrag = inputSystem.mouse.position;
        ...
        if (isPress && grab.holded)
        {
            ...
            {
                Vector2Int offset = inputSystem.mouse.position - grab.beginDrag;
