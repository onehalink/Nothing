...
namespace Engine::UI
{
	...
	*/
	...
	constexpr Int32 CodePointOffset = 32;
	...
	Void Text(IDrawable* interface, const Vector2Int& position, const U8String& text, const Color& color)
	{
		...
		{
			Char8 value = text[index];
			if ( value < CodePointOffset + 1 )
				continue;

			Byte* mask = LookupTable[ value - CodePointOffset ];
			for (SizeType x = 0; x < DefaultFontSizeInPixel.x; x++)
				for (SizeType y = 0; y < DefaultFontSizeInPixel.y; y++)
					if ( ( mask[y] >> x ) & 0x01 )
						interface->SetPixel(
							position.x + x + DefaultFontSizeInPixel.x * index,
							position.y + y,
							color
						);
		}
