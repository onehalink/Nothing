...
namespace Engine::UI
{
	...
	Void CircleProcessBar::Update(const InputSystem& inputSystem)
	{
		if (!active)
			return;
	}
	...
	Void CircleProcessBar::Render(IDrawable* interface)
	{
		...
		constexpr RealType Unit = TwoPi<RealType>;
		// 0 - 0.25
		{
		...
		if(value > 0.25)
		{
			RealType radian = (Minimum(value, 0.50 - Epsilon<RealType>) - 0.25) * Unit;
		...
		if(value > 0.75)
		{
			RealType radian = (Minimum(value, 1 - Epsilon<RealType>) - 0.75) * Unit;
			RealType k = Sine(radian) / -Cosine(radian);
