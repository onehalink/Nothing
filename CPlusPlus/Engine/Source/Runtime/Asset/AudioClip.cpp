...
namespace Engine
{
	...
	{
		MemoryCopy( audioFrame, ( *this )[index], audioFrameSize );
	}
	...
	AudioClip AudioClip::ConvertDataStoreType(DataStoreType specific) const
	{
		...
		case DataStoreType::Float32:
			if ( channelCount == 1 )
				OpA = [](Float32* output, const Void* data)
				{
					MemoryCopy( data, output, sizeof(Float32) * 1 );
				};
			else
				OpA = [](Float32* output, const Void* data)
				{
					MemoryCopy( data, output, sizeof(Float32) * 2 );
				};
			break;
		}
		...
		AudioClip result( specific, samples * ( GetBitDepth(specific) / 8 * channelCount ), sampleRate, channelCount );

		Float32 buffer[2];
		for (SizeType index = 0; index < samples; index++)
		{
			OpA( buffer,		( *this )[index]	);
			OpB( result[index],	buffer				);
		}
