...
namespace Engine
{
	...
	struct Vector : VectorBase<D, T>, VectorConstants<D, T, Vector<D, T>>
	{
		...
		Vector(Initialization initialization = Initialization::DoNothing);
	...
	template<Dimension D, typename T, typename S> Vector<D, T> Lerp(const Vector<D, T>& from, const Vector<D, T>& to, S scalar);

	/**
	* @remark
	* Dimension(D) must is 2 or 3
	*/
	...
	/**
	* @remark
	* Usage only if IsInteger<T>::Value
	*/
...
#pragma region Details
	...
	template<Dimension D, typename T>
	Vector<D, T>::Vector(Initialization initialization)
	{
		if ( initialization == Initialization::ForceToZeroMemory )
			Memory::Clear( this, sizeof( Vector<D, T> ) );
	}
	...
	Vector<D, T>::Vector(const Vector<E, U>& other, Parameters ...parameters)
	{
		...
		static_assert( sizeof...(parameters) <= Absolute( static_cast<Int32>( D - E ) ) );
	...
	Vector<D, T> Lerp(const Vector<D, T>& from, const Vector<D, T>& to, S scalar)
	{
		Vector<D, T> ret;
		for (SizeType index = 0; index < D; index++)
			ret[index] = Lerp( from[index], to[index], scalar );
	...
	Vector<D, T> operator/(const Vector<D, T>& vector, T scalar)
	{
		if constexpr ( IsInteger<T>::Value )
			ASSERT( scalar == 0, Error::DivisorIsZero );
		else
			ASSERT( Absolute(scalar) < Epsilon, Error::DivisorIsZero );
