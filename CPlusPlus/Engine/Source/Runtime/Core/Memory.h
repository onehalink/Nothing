...
namespace Engine
{
	// Represent a contiguous memory block that allocated on the heap
	template<typename T, typename Allocator = DefaultAllocator<T>>
	class Memory
	{
	public:
		Memory();
		Memory(SizeType requiredCount);
...
#pragma region Details
	...
	template<typename T, typename Allocator>
	Memory<T, Allocator>::Memory(SizeType requiredCount)
	...
	Void Memory<T, Allocator>::CopyFrom(const Memory<T, Allocator>& other)
	{
		...
		MemoryOp::Copy( other.buffer, buffer, sizeof(T) * count );
	}
