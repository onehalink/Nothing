...
namespace Engine
{
	template<typename T> Bool Contain(const T* rawString, T search, Int32 length = -1, Int32* outputMatchIndex = nullptr);
...
#pragma region Details
	...
	Bool Contain(const T* rawString, const T* search, Int32 length, Int32 searchLength, Int32* outputMatchIndex)
	{
		...
		for(SizeType index = 0; index <= end; index++)
			if ( MemoryOp::Equal( rawString + index, search, sizeof(T) * searchLength ) )
			{
				if (outputMatchIndex)
