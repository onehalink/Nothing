...
#pragma region Details
	...
	String<T> Insert(const T* rawString, SizeType startIndex, const T* value, Int32 length, Int32 valueLength)
	{
		...
		MemoryOp::Copy( value,					temporary.GetBuffer() + startIndex,					sizeof(T) * valueLength				);
		MemoryOp::Copy( rawString + startIndex,	temporary.GetBuffer() + startIndex + valueLength,	sizeof(T) * ( length - startIndex )	);
