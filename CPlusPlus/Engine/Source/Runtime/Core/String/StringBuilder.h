...
namespace Engine
{
	...
	public:
		...
		StringBuilder<T>& Append(const T* string, Int32 length = -1);
...
#pragma region Details
	...
	template<typename T>
	StringBuilder<T>& StringBuilder<T>::Append(const T* string, Int32 length)
	{
		if ( length == -1 )
		...
		MemoryCopy( string, GetBuffer() + count, sizeof(T) * length );
		count += length;
	...
	String<T> StringBuilder<T>::ToString() const
	{
		...
		MemoryCopy( GetBuffer(), result.GetBuffer(), sizeof(T) * count );
	...
	Void StringBuilder<T>::Expand(SizeType requiredCount)
	{
		...
		MemoryCopy( GetBuffer(), temporary.GetBuffer(), sizeof(T) * count );
