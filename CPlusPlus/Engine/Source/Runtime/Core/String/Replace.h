...
#pragma region Details
	...
	String<T> Replace(const T* rawString, const T* search, const T* replace, Int32 length, Int32 searchLength, Int32 replaceLength)
	{
		...
		Memory::Copy( replace,		temporary.GetBuffer() + startIndex, sizeof(T) * replaceLength	);

		SizeType offset = startIndex + searchLength;
		Memory::Copy( rawString + offset, temporary.GetBuffer() + startIndex + replaceLength, sizeof(T) * ( length - offset ) );
