...
namespace Engine
{
	...
	public:
		...
		// Chars had been contained(terminator excluded)
	...
	/**
	* @remark
	* The parameter [count] indicates that how many chars(terminator included) could be stored by a pointer(rawString) point to heap/stack memory
	* The last char should be rawString[count - 2]
	* The null terminator should be rawString[count - 1]
	*/
	template<typename T> Void SetNullTerminatorForRawString(T* rawString, SizeType count);
	template<typename T> Void SetNullTerminatorForRawString(Span<T>& rawStringWrapper);
	template<typename T> const T* SkipWhitespace(const T* rawString);
	template<typename T> SizeType StringLength(const T* rawString, T terminator = NullTerminator);
...
#pragma region Details
	...
	String<T>::String(const T* inUTF)
	{
		...
		SetNullTerminatorForRawString(storage);
	...
	String<T>::String(Span<T>&& rawStringWrapper)
		: storage( Forward<Span<T>>(rawStringWrapper) )
	{
	...
	{
		// bug fix when storage.GetCount() == 0
		if ( !GetBuffer() )
			return 0;
	...
	template<typename T>
	Void SetNullTerminatorForRawString(T* rawString, SizeType count)
	{
		rawString[ count - 1 ] = NullTerminator;
	}

	template<typename T>
	Void SetNullTerminatorForRawString(Span<T>& rawStringWrapper)
	{
		SetNullTerminatorForRawString( rawStringWrapper.GetBuffer(), rawStringWrapper.GetCount() );
	}

	template<typename T>
	const T* SkipWhitespace(const T* rawString)
	{
		while ( *rawString != NullTerminator )
			rawString++;
	...
	template<typename T>
	SizeType StringLength(const T* rawString, T terminator)
	{
		SizeType ret = 0;
		while ( rawString[ret] != terminator ) ret++;
