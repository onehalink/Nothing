...
namespace Engine
{
	...
	*/
	...
	Memory<Byte> Bitmap::Encode(const Texture& texture)
	{
		...
		{
			MemoryOp::Clear( ret.GetBuffer(), DefaultBitsOffset );
			MemoryOp::Copy( texture.storage.GetBuffer(), ret.GetBuffer() + DefaultBitsOffset, imageSize );
	...
	Texture Bitmap::Decode(const Void* buffer, SizeType byteCount)
	{
		...
		MemoryOp::Copy( reinterpret_cast<const Byte*>(buffer) + reinterpret_cast<const FileHeader*>(buffer)->offset, ret.storage.GetBuffer(), descriptor->imageSize );
