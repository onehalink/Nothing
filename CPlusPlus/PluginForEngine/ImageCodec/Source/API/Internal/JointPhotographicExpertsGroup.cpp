...
namespace Engine::Plugin
{
	...
	Memory<Byte> JointPhotographicExpertsGroup::Encode(const Texture& texture)
	{
		...
		Memory<Byte> ret(compressedImageSize);
		MemoryOp::Copy( span.GetBuffer(), ret.GetBuffer(), compressedImageSize );
	...
	Texture JointPhotographicExpertsGroup::Decode(const Void* buffer, SizeType byteCount)
	{
		...
		MemoryOp::Copy( outputImage, ret.storage.GetBuffer(), ret.storage.GetCount() );
