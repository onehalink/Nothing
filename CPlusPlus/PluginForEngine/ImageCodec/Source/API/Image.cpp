...
namespace Engine::Plugin
{
	...
	Span<Byte> Image::SaveToMemory(const Texture& texture, Format format, SizeType imageQualityFactor)
	{
		switch (format)
		{
		case Format::Bmp:
		...
		case Format::Jpg:
			return JointPhotographicExpertsGroup::Encode(texture, imageQualityFactor);
		case Format::Png:
			return PortableNetworkGraphics::Encode(texture);
		case Format::Webp:
			return WebPictureFormat::Encode(texture, imageQualityFactor);
		default:
			return Span<Byte>();
		}
	}

	Void Image::SaveToFile(const U8String& path, const Texture& texture, Format format, SizeType imageQualityFactor)
	{
		Span<Byte> image = SaveToMemory(texture, format, imageQualityFactor);
