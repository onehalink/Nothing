...
namespace Engine::Plugin
{
	...
	static PNGState GetPNGState(const Void* buffer, SizeType byteCount)
	{
		...
		UInt32		height;
		PNGState	state;
		lodepng_inspect( &width, &height, &state, reinterpret_cast<const Byte*>(buffer), byteCount );
	...
	Span<Byte> PortableNetworkGraphics::Encode(const Texture& texture)
	{
		...
		Byte*	output;
		UInt64	outputSize;
		lodepng_encode_memory( &output, &outputSize, temporary.storage.GetBuffer(), temporary.width, temporary.height, temporary.format == PixelFormat::R8G8B8 ? LCT_RGB : LCT_RGBA, 8 );
	...
	Texture PortableNetworkGraphics::Decode(const Void* buffer, SizeType byteCount)
	{
		...
		UInt32	width;
		UInt32	height;
		lodepng_decode_memory( &output, &width, &height, reinterpret_cast<const Byte*>(buffer), byteCount, colorType, 8 );
