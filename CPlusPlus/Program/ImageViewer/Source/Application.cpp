...
Void OnUpdate(RealType deltaTime)
{
	...
	{
		U16String selected;
		if (FileDialog::Open(&selected, { InPlace(), "*.jpg", "*.png", "*.webp", "*.bmp" }, Path::GetFullPathName("."), "Open image", runtime->window.GetHandle()))
		{
			runtime->imageLoaded = true;

			runtime->image = ImageExtension::LoadFromFile(selected);
			runtime->renderTexture = ImageExtension::ConvertTexturePixelFormat(runtime->image, PixelFormat::B8G8R8A8);
		}
		else
		{
	...
	// render
	{
		...
		for(SizeType y = yMin; y < yMax; y++)
			for (SizeType x = xMin; x < xMax; x++)
			{
				...
				reinterpret_cast<UInt32*>(RenderTarget)[width * y + x] = ToB8G8R8A8(FromR8G8B8(rgb24));
				*/

				reinterpret_cast<UInt32*>(RenderTarget)[width * y + x] = 
					runtime->renderTexture.GetPixel
					(
						(imageWidth - 1) * u,
						(imageHeight - 1) * v
					);
